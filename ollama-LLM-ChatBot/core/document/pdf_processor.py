...
class PDFProcessor:
    ...
    def __init__(self, 
        ...
        if self.enable_keyword_extraction:
            ...
            logger.info(f"✅ 키워드 추출 기능 활성화 (임계값: {keyword_cache_threshold})")
            ...
            for line in lines:
                ...
                if current_length + len(line) > self.chunk_size:
                    if current_chunk:
                        # 현재 청크 저장
                        chunk_content = '\n'.join(current_chunk)
                        chunk_id = self._generate_chunk_id(chunk_content, pdf_id, len(chunks))
    ...
    def _extract_keywords(self, chunks: List[TextChunk], pdf_id: str):
        ...
        with memory_profiler(f"키워드 추출: {pdf_id or 'unknown'}"):
            try:
                ...
                # 키워드 추출
                keywords = self.keyword_extractor.extract_keywords_from_texts(texts)
                
                # 메타데이터에 키워드 추가
                for chunk in chunks:
                    if chunk.metadata is None:
                        chunk.metadata = {}
                    chunk.metadata["keywords"] = keywords.get(chunk.chunk_id, [])
                
                logger.debug(f"키워드 추출 완료: {len(keywords)}개 키워드")
