...
import { TrafficVolumeData, TrafficData } from '../../types/global.types';
...
export const ChartDemo: React.FC = () => {
  ...
  const handleChartReady = (chartType: string) => (imageData: string) => {
    ...
    console.log(`Chart image generated for ${chartType}:`, imageData.substring(0, 50) + '...');
  };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {/* Chart Type Selector */}
      <div className="mb-8">
        <h2 className="text-xl font-semibold mb-4">Selector de Tipo de Gráfico</h2>
        <div className="flex flex-wrap gap-2">
          {(['bar', 'line', 'combined', 'analysis'] as ChartType[]).map((type) => (
            <button
              key={type}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="border border-gray-200 rounded-lg p-4 bg-white">
          ...
          {generatedImages['combined'] && (
            <div className="mt-4 p-2 border rounded">
              <img src={generatedImages['combined']} alt="Combined Chart Preview" />
            </div>
          )}
        </div>
        ...
        <div className="border border-gray-200 rounded-lg p-4 bg-white">
          ...
          {generatedImages['volume-only'] && (
            <div className="mt-4 p-2 border rounded">
              <img src={generatedImages['volume-only']} alt="Volume Chart Preview" />
            </div>
          )}
        </div>
      </div>
      ...
      <div className="mt-12 grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
          <h3 className="text-lg font-semibold mb-3">Datos de Volumen por Dirección</h3>
        ...
        <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
          <h3 className="text-lg font-semibold mb-3">Datos Temporales (Muestra)</h3>
          <pre className="text-sm text-gray-700 overflow-x-auto max-h-64 overflow-y-auto">
            ...
            <div className="text-center text-gray-500 mt-2">... y {sampleTimeData.length - 5} más</div>
          </pre>
        </div>
      </div>
    </div>
